...
final class SigningHandler: NSObject {
	...
	func copy() async throws {
		...
		let movedAppURL = _uniqueWorkDir.appendingPathComponent(
		...
		Logger.misc.info(
			"[\(self._uuid)] Moved Payload to: \(movedAppURL.path)"
		)
	}
	...
	func modify() async throws {
		...
		try await _modifyDict(
			using: infoDictionary,
			with: _options,
			to: movedAppPath
		)
		...
		if !_options.removeFiles.isEmpty {
			try await _removeFiles(
				for: movedAppPath,
				from: _options.removeFiles
			)
		}
		...
		if let identifier = _options.appIdentifier,
			let oldIdentifier,
			oldIdentifier != identifier
		{
			try await _modifyPluginIdentifiers(
				old: oldIdentifier,
				new: identifier,
				for: movedAppPath
			)
		...
		if _options.experiment_replaceSubstrateWithEllekit {
			...
			if !_options.injectionFiles.isEmpty
				|| !_options.injectionFrameworks.isEmpty
			{
				try await _inject(for: movedAppPath, with: _options)
			}
		}
		...
		if _options.signingOption == .default,
			appCertificate != nil
		{
	...
	func move() async throws {
		...
		destinationURL = destinationURL.appendingPathComponent(
			movedAppPath.lastPathComponent
		)
	...
	func addToDatabase() async throws {
		...
		await withCheckedContinuation {
			...
			Storage.shared.addSigned(
				uuid: _uuid,
				source: _app.source,
				certificate: _options.signingOption != .default
					? nil : appCertificate,
				appName: bundle?.name,
...
extension SigningHandler {
	private func _modifyDict(
		...
		if options.fileSharing {
			infoDictionary.setObject(
				true,
				forKey: "UISupportsDocumentBrowser" as NSCopying
			)
		}
		if options.itunesFileSharing {
			infoDictionary.setObject(
				true,
				forKey: "UIFileSharingEnabled" as NSCopying
			)
		}
		if options.proMotion {
			infoDictionary.setObject(
				true,
				forKey: "CADisableMinimumFrameDurationOnPhone" as NSCopying
			)
		}
		if options.gameMode {
			infoDictionary.setObject(
				true,
				forKey: "GCSupportsGameMode" as NSCopying
			)
		}
		if options.ipadFullscreen {
			infoDictionary.setObject(
				true,
				forKey: "UIRequiresFullScreen" as NSCopying
			)
		}
		...
		if options.appAppearance != .default {
			infoDictionary.setObject(
				options.appAppearance.rawValue,
				forKey: "UIUserInterfaceStyle" as NSCopying
			)
		}
		if options.minimumAppRequirement != .default {
			infoDictionary.setObject(
				options.minimumAppRequirement.rawValue,
				forKey: "MinimumOSVersion" as NSCopying
			)
		}
		...
		if let customIdentifier = options.appIdentifier {
			infoDictionary.setObject(
				customIdentifier,
				forKey: "CFBundleIdentifier" as NSCopying
			)
		}
		if let customName = options.appName {
			infoDictionary.setObject(
				customName,
				forKey: "CFBundleDisplayName" as NSCopying
			)
			infoDictionary.setObject(
				customName,
				forKey: "CFBundleName" as NSCopying
			)
		}
		if let customVersion = options.appVersion {
			infoDictionary.setObject(
				customVersion,
				forKey: "CFBundleShortVersionString" as NSCopying
			)
			infoDictionary.setObject(
				customVersion,
				forKey: "CFBundleVersion" as NSCopying
			)
		}
	...
	private func _modifyLocalesForName(_ name: String, for app: URL)
		async throws
	{
	...
	private func _modifyPluginIdentifiers(
		...
		for bundleURL in pluginBundles {
			...
			if let oldValue = infoDict["WKCompanionAppBundleIdentifier"]
				as? String
			{
				let newValue = _replaceIfPrefix(
					oldValue,
			...
			if let extensionDict = (infoDict["NSExtension"] as? NSDictionary)?
				.mutableCopy() as? NSMutableDictionary
			{
				// NSExtension → NSExtensionAttributes → WKAppBundleIdentifier
				if let attributes =
					(extensionDict["NSExtensionAttributes"] as? NSDictionary)?
					.mutableCopy() as? NSMutableDictionary,
					let oldValue = attributes["WKAppBundleIdentifier"]
						as? String
				{
					let newValue = _replaceIfPrefix(
						oldValue,
				...
				if let oldValue = extensionDict[
					...
					if oldValue != newValue {
						extensionDict["NSExtensionFileProviderDocumentGroup"] =
							newValue
						didChange = true
					}
				}
	...
	private func _injectAppExtensions(for app: URL, with options: Options)
		async throws
	{
		guard !options.injectedAppExtensions.isEmpty else { return }

		let pluginsDirectory = app.appendingPathComponent(
			"PlugIns",
			isDirectory: true
		)
		...
		for extensionURL in options.injectedAppExtensions {
			...
			let destinationURL = pluginsDirectory.appendingPathComponent(
				extensionURL.lastPathComponent
			)

			if let collisionURL = _locateExtensionCollision(
				for: metadata,
				in: app
			) {
	...
	private func _locateExtensionCollision(
		for injected: AppExtensionMetadata,
		in app: URL
	) -> URL? {
		...
		for existingURL in existingBundles {
			...
			let isIdentifierMatch =
				...
				&& existingMetadata.bundleIdentifier
					== injected.bundleIdentifier
	...
	private func _removeCodeSigningArtifacts(in appex: URL) async throws {
		let embeddedProvisioning = appex.appendingPathComponent(
			"embedded.mobileprovision"
		)
		let signatureDirectory = appex.appendingPathComponent(
			"_CodeSignature",
			isDirectory: true
		)
		...
		let nestedArtifacts = _enumerateFiles(at: appex) {
			$0.hasSuffix("_CodeSignature")
				|| $0.hasSuffix("embedded.mobileprovision")
		}
	...
	private func _replaceIfPrefix(_ value: String, old: String, new: String)
		-> String
	{
	...
	private func _removeFiles(for app: URL, from appendingComponent: [String])
		async throws
	{
	...
	private func _locateMachosAndChangeToSDK26(for app: URL) async throws {
		if let url = Bundle(url: app)?.executableURL {
			LCPatchMachOForSDK26(
				app.appendingPathComponent(url.relativePath).relativePath
			)
		}
	}

	private func _locateCodeSignatureDirectories(for app: URL) async throws
		-> [URL]
	{
...
enum SigningFileHandlerError: Error, LocalizedError {
	...
	var errorDescription: String? {
		...
		case .invalidInjectedExtension:
			"One or more injected extensions are invalid."
		}
	}
}
